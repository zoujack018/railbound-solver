...
function applyTunnel(nx, ny, ne, tm) { return null; }
...
function zeroSafetyLookahead(puzzle, cars, ctx) {
  ...
  for (let i = 0; i <= steps; i++) {
    ...
    for (const c of nxt) {
      const k = pk(c.x, c.y);
      if (occ.has(k)) return { ok: false, reason: "碰撞", detail: { errorCode: 'CELL_COLLISION', cars: [occ.get(k), c.name], cell: k } };
      occ.set(k, c.name);
      if (tm[k]) {
        const pkPair = pk(tm[k].pair.x, tm[k].pair.y);
        if (occ.has(pkPair)) return { ok: false, reason: "碰撞", detail: { errorCode: 'TUNNEL_COLLISION', cars: [occ.get(pkPair), c.name], cells: [k, pkPair] } };
        occ.set(pkPair, c.name);
      }
    }
    ...
    for (const color of triggeredColors) if (!blocked.has(color)) toggled[color] = !toggled[color];
    const tsTriggeredColorSet = new Set(tsTriggeredColors);
    for (const c of zeroCars) {
      const k = pk(c.x, c.y), sw = tswitchMap[k];
      if (sw && tsTriggeredColorSet.has(sw.color)) tsLocks[c.name] = { k, track: effectiveTSwitchTrack(sw, tsToggled) };
    }
    for (const color of tsTriggeredColors) tsToggled[color] = !tsToggled[color];
...
function simulate(puzzle, placed) {
  ...
  for (let t = 1; t <= ms; t++) {
    ...
    for (const c of nxt) {
      const k = pk(c.x, c.y);
      if (occ.has(k)) { const other = nxt.find(o => o !== c && pk(o.x, o.y) === k); return { ok: false, reason: "碰撞", arrived, steps: t, history, detail: { errorCode: 'CELL_COLLISION', cars: [other ? other.name : '?', c.name], cell: k } }; } occ.add(k);
      if (tm[k]) {
        const pkPair = pk(tm[k].pair.x, tm[k].pair.y);
    ...
    for (const color of triggeredColors) {
      if (!blocked.has(color)) toggled[color] = !toggled[color];
    }
    const tsTriggeredColorSet = new Set(tsTriggeredColors);
    for (const c of cars) {
      const k = pk(c.x, c.y), sw = tswitchMap[k];
...
function filterBlanks(pz) {
  ...
  for (const b of pz.blanks) {
    ...
    for (const en of fd) for (const tr of TRACKS_BY_ENTRY[en]) { const ex = exitPort(tr, en); if (ex && (!bd || bd.has(ex))) vt.add(tr); }
    const vb = new Set();
    for (const en of fd) for (const tr of BASIC_TRACKS) { const ex = exitPort(tr, en); if (ex && (!bd || bd.has(ex))) vb.add(tr); }
...
export {
  ...
  TRACKS_BY_ENTRY, BASIC_BY_ENTRY, TRACKS_BY_EXIT, TUNNEL_COLORS, BARRIER_COLORS, buildTunnelMap, applyTunnel,
